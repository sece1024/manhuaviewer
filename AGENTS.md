...
- **DNS-rebinding guard** (`routes/auth.rs::security_guard`, wired as the outermost `Router::layer` in `routes/mod.rs` — after `.fallback(...)`, so it also wraps the SPA fallback): Host/Origin must be a "local form" hostname — literal IP, `localhost`/`*.localhost`, `*.local`, or a single-label LAN name — otherwise **403** (`host_allowed`/`origin_allowed`/`request_host_origin_ok`, pure and unit-tested). Consequence: users must access via IP or short LAN names, not public/DDNS domains. Related loopback-only rules: non-loopback peers get **403** when PUTting `/api/settings` with `server_token`/`server_bind` (`settings.rs::setting_key_writable`), `POST /api/restore` validates archive type ↔ extension consistency before import (`settings.rs::validate_import_payload`), and `GET /api/archives/:id/file` verifies magic bytes before streaming (`services/archive.rs::magic_matches`).
...
- **Cross-machine sync** (`routes/sync.rs`): the local backend acts as client — pulls the peer's manifest + archives + metadata, then **pushes a tag mirror** back via `POST /api/sync/push` as the job's final phase (`push_tag_mirror` → `db/tags.rs::mirror_tags_by_title`). Merge semantics are **replace-by-title**: the machine that *runs* the sync is the tag source of truth (deletions propagate; unknown titles are ignored; same-title volumes share one set — title is the cross-machine key, IDs never travel, `TagRef` carries `namespace/name/color`). Payload limits + color sanitization in `validate_tag_mirror`/`sanitize_tag_color`; push failure is recorded in `job.failed` and never fails the download phase.
...
Versions live in three places and must be kept in sync: `package.json`, `src-tauri/tauri.conf.json`, `src-tauri/Cargo.toml`. **One command: `./scripts/release.sh <x.y.z>`** — runs `./scripts/bump-version.sh` + `pnpm changelog`, then commits *only* the three version files + `CHANGELOG.md` (never `git add -A`), tags `v<x.y.z>` and pushes (`-y` skips the confirm prompt; `--no-push` stops after the local commit+tag). It refuses to run off-main, with a dirty tree (the intentionally-uncommitted `src-tauri/gen/schemas/*` churn is exempt), without `git-cliff` on PATH, when the tag already exists, or when the version equals the current one. Manual equivalent: `./scripts/bump-version.sh` → `pnpm changelog` (git-cliff, e.g. `brew install git-cliff`) → commit → tag → push. Pushing the tag triggers `.github/workflows/release.yml`, which builds macOS arm64 + Windows x64 installers and creates a **draft** GitHub Release whose body is generated by git-cliff (`--latest --strip header`, config in `cliff.toml`) (manually publish from the Releases page). Full flow: see `CONTRIBUTING.md`.
