    ...
    match db.get_archive(id) {
        Ok(Some(archive)) => {
            match crate::services::archive::create_archive_reader(
                ...
                Ok(reader) => match reader.get_cover() {
                    Ok(cover_data) => {
                        (StatusCode::OK, [("Content-Type", "image/jpeg")], cover_data)
                            .into_response()
                    }
                    Err(e) => error_response(StatusCode::INTERNAL_SERVER_ERROR, &e.to_string()),
                },
                ...
                Ok(reader) => {
                    ...
                    if (page_index as usize) < pages.len() {
                        ...
                        match reader.extract_page(page_name) {
                            Ok(data) => {
                                let thumb_gen =
                                    crate::services::thumbnail::ThumbnailGenerator::default();
...
pub async fn open_file(
    ...
    let page_count =
        match crate::services::archive::create_archive_reader(&payload.file_path, &archive_type) {
