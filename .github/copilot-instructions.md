...
- **Backend** (`src-tauri/src/`): `routes/` (Axum handlers: archives, tags, categories, history, settings, scan, convert, sync, metadata, update, opds, plus `auth.rs` middleware), `services/` (`archive.rs` extraction, `scanner.rs`, `thumbnail.rs`, `cbz.rs`, `backup.rs`, `cleanup.rs`, `metadata.rs`, `cache_budget.rs`, `page_cache.rs`, `fs_ext.rs`), `db/` (rusqlite wrapper + schema + migrations), `logging.rs` (file logging, panic hook).
- **Frontend** (`frontend/src/`): React 19 + React Router v7, built with Vite (Jest tests still run through `react-scripts test`). Pages: `Library`, `Reader`, `History`, `Settings`. Shared hooks in `hooks/` (`useSettings`, `useTags`, `useReaderKeyboard`, `useGamepad`, `useScan`, `useSync`, `useCbzConvert`, `useLibrarySession`, …), all API calls through `utils/api.js`.
- **Database**: SQLite via rusqlite at `~/Library/Application Support/MangaViewer/data/manhuaviewer.db` (macOS; other platforms via `dirs::data_dir()`). Overridable via `DATA_DIR` env var (use it for isolated dev/test runs); HTTP port overridable via `PORT`. `db/mod.rs` holds the `Database` struct + r2d2 pool + shared helpers; SQL queries are split into per-domain submodules (`db/archives.rs`, `db/tags.rs`, `db/categories.rs`, `db/history.rs`, `db/settings.rs`, `db/bookmarks.rs`, `db/backup.rs`); `db/schema.rs` is the canonical idempotent schema; `db/migrations.rs` migrates legacy tables (`folders`, `folder_tags`, `read_history`) and adds columns.
...
- **State sharing**: `AppState` (defined in `main.rs`) wraps `db: Arc<Database>` (an **r2d2 pool — no `Mutex`**), `data_dir`, and `last_thumb_eviction`. It's shared via Axum's `with_state(Arc<AppState>)`; DB calls are offloaded through `run_db`/`db_json` in `routes/mod.rs`.
...
- **Search/filter semantics**: search is a server-side `LIKE` match against **title OR tag name** (space-separated terms are AND-ed, a `-term` prefix excludes); tag filters accept `namespace:name` syntax; categories are either static (join table) or dynamic (a stored `search` expression matched against title).
...
- If `pnpm install` fails with `ERR_PNPM_IGNORED_BUILDS` for a native package, whitelist it in `pnpm-workspace.yaml`, which this repo configures via the `allowBuilds:` map (e.g. `core-js: true`) — an earlier placeholder-valued block was invalid and removed (`a73c160`), then re-added as a proper map (`ee4284f`); don't "fix" it back to `onlyBuiltDependencies` without checking your pnpm version's docs.
...
- The CSP in `src-tauri/tauri.conf.json` allows `unsafe-inline` for styles/scripts (no `unsafe-eval`) and adds `object-src 'none'`/`base-uri 'none'`/`frame-ancestors 'none'` — don't tighten it without testing a dev build.
